...
/**
 * Fetch a proposal by its creation transaction hash
 */
export async function fetchProposalByTxHash(
  provider: ethers.providers.Provider,
  txHash: string,
  ...
  try {
    ...
    for (const log of receipt.logs) {
      ...
      try {
        ...
        if (parsed.name === "ProposalCreated") {
          ...
          const proposalValues = parsed.args[3] as ethers.BigNumber[];
    ...
    debug.search("failed to fetch proposal by tx hash %s: %O", txHash, e);
    return null;
  }
}

/**
 * Fetch proposals from bundled cache seeds
 * Retrieves proposal details from creation tx hashes
 */
export async function fetchProposalsFromSeeds(
  provider: ethers.providers.Provider,
  seeds: Array<{
    proposalId: string;
    governorAddress: string;
  ...
  const queries = seeds.map((seed) => async () => {
    // Fetch full proposal data from tx receipt
    const proposal = await fetchProposalByTxHash(
      provider,
      seed.creationTxHash,
      seed.governorAddress
    );

    if (!proposal) return null;

    // Fetch current state and votes
    const contract = getContract(seed.governorAddress);
    try {
      const stateData = await fetchProposalStateAndVotes(
        contract,
        proposal.id,
      ...
      debug.search("failed to fetch state for seeded proposal: %O", e);
      return null;
    }
  });
